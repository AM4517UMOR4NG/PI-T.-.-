...
@section('sidebar_menu')
    ...
    <a href="{{ route('admin.damage-reports.index') }}" class="nav-link {{ request()->routeIs('admin.damage-reports.*') ? 'active' : '' }}">
        <i class="bi bi-exclamation-triangle"></i>
        <span>Laporan Kerusakan</span>
        @php
            $pendingCount = \App\Models\DamageReport::where('status', 'pending')->count();
        @endphp
        @if($pendingCount > 0)
